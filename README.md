...
탐색 시스템을 사용하면 작업을 통해 탐색할 수도 있다. 작업별 탐색은 대상별 탐색과 비교해 다음과 같은 이점이 있다.

- 앱에서 탐색 경로를 시각화 할 수 있다.
- 작업에는 전환 애니메이션, 인수 값, 백 스택 동작 등 개발자가 설정할 수 있는 추가 연결 속성이 포함될 수 있다.
- safe args 플러그인을 사용하여 탐색할 수 있으며 이 내용은 잠시 후에 알아보자.
- 동일 ID action 을 설정하여 작업의 추상화 수준을 제공하고 컨텍스트에 따라 개발자를 다른 위치로 이동할 수 있는 방법을 제공한다.
- `popUpTo` 속성 : 대상에 도달할 때까지 백 스택에서 프래그먼트를 표시한다.
